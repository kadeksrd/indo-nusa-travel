...
export default function BookingModal({ paket, open, onClose }: Props) {
  ...
  const totalHarga = hargaSatuan * form.jumlah_orang;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Step 2: Redirect ke halaman pembayaran
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-auto">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-sm font-medium text-gray-700 flex items-center gap-1 mb-1">
                <Users className="w-3 h-3" /> Jumlah Orang{" "}
                <span className="text-red-500">*</span>
              </label>
              <input
                type="number"
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 flex items-center gap-1 mb-2">
              <CreditCard className="w-4 h-4" /> Pilihan Pembayaran{" "}
              <span className="text-red-500">*</span>
            </label>
            <div className="grid grid-cols-2 gap-3">
              {/* DP */}
              ...
              >
                ...
                <p className="text-blue-700 font-bold mt-1">
                  {formatHarga(jumlahDP)}
                </p>
                <p className="text-xs text-gray-400 mt-0.5">
                  Sisa: {formatHarga(totalHarga - jumlahDP)}
                </p>
              </button>
          ...
          <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm border">
            ...
            <div className="flex justify-between text-gray-600">
              <span>
                {formatHarga(hargaSatuan)} × {form.jumlah_orang} orang
              </span>
            ...
            <div className="border-t pt-2 flex justify-between font-bold text-gray-900">
              ...
              <span className="text-blue-700 text-base">
                {formatHarga(jumlahBayar)}
              </span>
            </div>
            {form.tipe_pembayaran === "dp" && (
              <p className="text-xs text-orange-600">
                * Sisa {formatHarga(totalHarga - jumlahDP)} dibayar sebelum
                keberangkatan
              </p>
            )}
          </div>
          ...
          >
            {loading ? (
              ...
              `Lanjut ke Pembayaran → ${formatHarga(jumlahBayar)}`
            )}
          </button>
        </div>
      </div>
